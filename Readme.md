...
- `backend` (optional, defaults to `single`): The backend you want to use for running the experiments. Currently supported are {`multiprocessing`, `ray`, `single`}. Any string which is not `multiprocessing` or `ray` will be interpreted as `single`. As the names suggest, `multiprocessing` will run experiment on the same machine using multiple processes (using a `multiprocessing.Pool`). `ray` uses [Ray](https://github.com/ray-project/ray) to distribute experiments across multiple machines and `single` just runs one experiment after another on the current machine without multi-threading.
